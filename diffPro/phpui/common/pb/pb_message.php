            ...
            if (is_array($this->values[$index]) && count($this->values[$index]) > 0)
            {
                ...
                foreach ($this->values[$index] as $array)
                {
                    if(!method_exists($array,'SerializeToString')){
                        $debugInfo = array();
                        $debugInfo['message'] = $this->getClassNameWithoutNamespace($this);
                        $debugInfo['index'] = $index;
                        $debugInfo['fieldNames'] = self::$fieldNames[$this->getClassNameWithoutNamespace($this)][$index];
                        $debugInfo['index_content'] = json_encode($this->values[$index]);
                        $debugInfo['wrong_content'] = json_encode($array);
                        $monitorLog = new Common_Log();
                        $monitorLog->write('MONITOR','wrongFormat',$debugInfo);
                        continue;
                    }
                    $newstring = '';
