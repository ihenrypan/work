...
int jsonDiff(FILE* fout, Value* onValue, Value* offValue, int* status, string path) {
    ...
    switch (onType) {
                ...
                if (offValue->HasMember(key)) {
                    ...
                    if (status == 2) {  // exist both in onValue && offValue
                        fprintf(fout, "%s->%s|%s|%s\n", path.c_str(), key, GetValueToString(tmpOnValue).c_str(),
                                GetValueToString(tmpOffValue).c_str());
        ...
        case kStringType: {
            // compare two string
            if (onValue->GetStringLength() != offValue->GetStringLength()
                    || strcmp(onValue->GetString(), offValue->GetString()) != 0) {
                *status = 2;
            } else {
                *status = 3; // equal
            }
        ...
        case kNumberType: {
            ...
            if (onIsInt == offIsInt) {
                if ((onIsInt = 1 && onValue->GetInt64() == offValue->GetInt64())
                        || (onValue->GetDouble() == offValue->GetDouble())) {
                    *status = 3;
                } else {
                    *status = 2;
                }
